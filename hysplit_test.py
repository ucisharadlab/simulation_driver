...
def test(test_name: str, param_values: list, attempts: int, output_dir: str = "./hysplit_out"):
...
def pollutants_test(start=0, end=500, step=50, attempts=1):
    default_pollutant = {
                "%id%": "0000",
                "%emission_rate%": "50.0",
...
def output_grid_spacing_test(start=0.1, end=3.0, step=0.1, attempts=1):
    ...
    test_values = [(0.01, [default_grid.copy()])]
    ...
    test("spacing", test_values, attempts)
...
def output_grid_span_test(start=0.0, lat_end=180.0, long_end=360.0, step=10, attempts=1):
    ...
    test("span", test_values, attempts)
...
def output_grid_sampling_test(start=0, end=60, step=5, attempts=1):
    ...
    test_values = [(0.01, {param_key: [default_grid.copy()]})]
    ...
    for rate in range(2, 49, 4):
        default_grid["%sampling%"] = f"00 00 00 00 00\n00 00 00 00 00\n00 {rate:02d} 00"
        test_values.append((rate * 60, {param_key: [default_grid.copy()]}))
    test("sampling", test_values, attempts)
